...
class EditForm(tk.Frame):
    ...
    def create_window(self, window):
        ...
        self.frame.title("Edytowanie obiektu")
        self.frame.geometry("400x600")
        self.frame.minsize(400, 600)
        self.frame.maxsize(400, 600)
        self.frame.config(bg=PALETTE["BACKGROUND"])

        self.frame.columnconfigure([0, 1], weight=1)
        self.frame.rowconfigure([0, 1, 2, 3, 4], weight=1)

        header = SectionHeader(self.frame, "Trwa edycja obiektu")
        header["font"] = FONTS["NORMAL_TEXT"]
        self.frame.update_idletasks()
        header.place(
            x=self.frame.winfo_width() / 2,
            y=self.frame.winfo_y() + 20,
            anchor="center",
        )

        name_input = Input(self.frame)
        identifier_input = Input(self.frame)
        unit_input =  tk.OptionMenu(self.frame, variable, "one", "two", "three")
        unit_cost_input = Input(self.frame)

        name_input.grid(row=0, column=0)
        identifier_input.grid(row=1, column=0)
        unit_input.grid(row=2, column=0)
        unit_cost_input.grid(row=3, column=0)

        confirm_btn = Button(
            self.frame, "Zapisz", "PRIMARY", command=self.confirm
        )
        confirm_btn.grid(row=1, column=0, sticky=tk.W, padx=10, pady=10)

        cancel_btn = Button(self.frame, "Anuluj", "DANGER", command=self.cancel)
